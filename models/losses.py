...
class PerceptualLoss():
	def contentFunc(self):
		...
		for i,layer in enumerate(list(cnn)):
			...
			if i == conv_3_3_layer:
				break
		return model                 #截取0~14层的VGG19赋予model
		
	def __init__(self, loss):
		self.criterion = loss  #损失函数
		self.contentFunc = self.contentFunc()	#获取截取的model
...
##############################################################################################
##############################################################################################
# class CyclePerceptualLoss():
# 	def cyclecontentFunc(self):
# 		model=UnetGenerator(3, 3, 7, 64, norm_layer='batch', use_dropout=True).to('cuda')
...
#
#
# net=UnetGenerator(3, 3, 7, 64, norm_layer=nn.BatchNorm2d, use_dropout=True).to('cuda')
# for i ,j in net.state_dict().items():
# 	print(i)
# 	print(j.shape)