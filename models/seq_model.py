...
class SeqModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.set_defaults(norm='batch', netG='resnet_9blocks', dataset_mode='aligned3')
        if is_train:
    ...
    def __init__(self, opt):
        ...
        self.netG=networks.define_G(6, opt.output_nc, opt.ngf, opt.netg1, opt.is_second_train, opt.norm)
        ...
        if self.isTrain:
            ...
            self.optimizer_G3 = torch.optim.Adam(
                ...
                betas=(opt.beta1, 0.999))
            self.optimizer_D0 = torch.optim.Adam(
                itertools.chain(self.netD0.parameters()), lr=opt.lr,
                betas=(opt.beta1, 0.999))
            self.optimizer_D1 = torch.optim.Adam(
                itertools.chain(self.netD1.parameters()), lr=opt.lr,
                betas=(opt.beta1, 0.999))
            self.optimizer_D2 = torch.optim.Adam(
                itertools.chain(self.netD2.parameters()), lr=opt.lr,
                betas=(opt.beta1, 0.999))
    ...
    def set_input(self, input):
        ...
        self.real_S = input['S' if AtoB else 'S'].to(self.device)
    ...
    def forward(self):
        ...
        self.x=self.netG(torch.cat((self.real_A, self.real_S),1))
    ...
    def backward_D(self):
        if  self.pic_num%3==0:
            ...
            self.loss_D1_fake, self.loss_D1_real = self.backward_D_basic(self.netD1, self.real_A, self.fake_B1,
                                                                     self.real_B1)
            self.loss_D = (self.loss_D1_fake + self.loss_D1_real) * 0.5
            self.loss_D.backward()
        elif self.pic_num%3==2:
            self.loss_D2_fake, self.loss_D2_real = self.backward_D_basic(self.netD2, self.real_A, self.fake_B2,
                                                                     self.real_B2)
    ...
    def backward_G(self):
        ...
        if self.pic_num%3==1:
            self.loss_G1_GAN, self.loss_G1_L1, self.loss_G1_perceptual = self.backward_G_basic(self.netD1, self.real_A,
                                                                                           self.fake_B1, self.real_B1)
            self.loss_G = self.loss_G1_GAN + self.loss_G1_L1 + self.loss_G1_perceptual
            self.loss_G.backward()
        if self.pic_num%3==2:
            self.loss_G2_GAN, self.loss_G2_L1, self.loss_G2_perceptual = self.backward_G_basic(self.netD2, self.real_A,
